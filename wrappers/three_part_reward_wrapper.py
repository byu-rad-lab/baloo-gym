...
class ThreePartRewardWrapper(gym.Wrapper):
    ...
    def calculate_reward(self):
        ...
        # joint_penalty = -(
        #     np.linalg.norm(right_j0)**2 + np.linalg.norm(right_j1)**2 +
        #     np.linalg.norm(right_j2)**2 + np.linalg.norm(left_j0)**2 +
        #     np.linalg.norm(left_j1)**2 + np.linalg.norm(left_j2)**2)

        # reward += joint_penalty

        #penalize chest being far from the box
