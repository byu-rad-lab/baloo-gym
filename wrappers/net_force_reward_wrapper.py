...
class NetForceRewardWrapper(gym.Wrapper):
    """
    ...
    def calculate_reward(self):
        ...
        reward = R_approach + R_netforceonbox + R_body
        ...
        # #penalize large net contact forces on body
        # contact_forces = get_contact_forces_on_body(self.env.unwrapped.model,
        #                                             self.env.unwrapped.data,
        #                                             "box")
        # net_force = np.sum(contact_forces, axis=0)
        # #normalize net force components to be between 0 and 1
        # net_force = net_force / np.max(net_force) if np.max(
        #     net_force) != 0 else net_force

        # r_grasp = -np.linalg.norm(net_force)
        r_grasp = 0
        ...
        r_box = np.maximum(r_box,
                           0)  # to avoid punishing agent for knocking box over
