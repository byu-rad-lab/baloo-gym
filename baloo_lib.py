...
def set_joint_angles(model, data, side, jangles):
    ...
    print(model.joint())
    #get associated dofs
    #split joint angles over the number of dofs for the whole disk
    #set the dofs with the split joint angles
...
def get_joint_angles(model, data, side):
    jangles = []
    for i in range(3):
        x = data.sensordata[model.sensor(f"{side}_u{i}").id]
        y = data.sensordata[model.sensor(f"{side}_v{i}").id]
        jangles.append(x)
        jangles.append(y)
...
def get_joint_vel(model, data, side):
    jvel = []
    for i in range(3):
        xd = data.sensordata[model.sensor(f"{side}_ud{i}").id]
        yd = data.sensordata[model.sensor(f"{side}_vd{i}").id]
        jvel.append(xd)
        jvel.append(yd)
